...
# table on d172 devlab contains records for date 2019-05-13 (older PSO v1)  and 2019-08-12 (newer PSO v1) and data for 2020y
# this extra filter is for demonstration to simulate error:  org.apache.spark.sql.AnalysisException: No such struct field installationLocation
...
df_al_json = df_pso.withColumn('json_data', F.from_json(F.col('jsonstruct'), json_schema_full.schema)) \
    ...
    # this will work if JSON schema STRUCT know-contains attributes, older PSO v1
    F.col('json_data.installationAddress.klsId').alias('klsid_ps_0'),         # older PSO,  before Jun 2019
    # this will work ONLY if JSON schema STRUCT know-contains ALL attributes, older PSO v1, + newer PSO v1
    F.col('json_data.installationLocation.address.klsId').alias('klsid_ps_1')  # newer PSO,  after Jun 2019
)

'''
# if the JSON-Schema knows only older PSO v1, (when data extra-filter (df_pso['acl_dop'] < '20190601000000') is ON)
# then this parsing/select attribute from json > select ( F.col('json_data.installationLocation.address.klsId').alias('klsid_ps_1')
# will failed , because JSON-Schema don't know this [.installationLocation] atrribute-field

py4j.protocol.Py4JJavaError: An error occurred while calling o106.select.
: org.apache.spark.sql.AnalysisException: No such struct field installationLocation in  .........

# so its necessary to build the JSON-schema from ALL available PSO records from source AL-table
'''
...
select max(acl_dop_iso) from db_d172_bbe_core_iws.cl_x_presalesorder_tmp  where klsid_ps_0 is not null;  --older PSO v1,  2019-05-13 12:38:45.0
select min(acl_dop_iso) from db_d172_bbe_core_iws.cl_x_presalesorder_tmp  where klsid_ps_1 is not null;  --newer PSO v1,  2019-08-12 09:55:57.0
